...
class Program
{
    static void Main(string[] args)
    {
        ...
        Item force_multiplier = new Item(new string[] { "force_multiplier" }, "a very weird looking artifact", "it is said to be able to output a great ammount of pressure on a very small point of contact surface, making it very deadly");
        ...
        Command action = new Move_Command();
        ...
        do
        {
            ...
            if (inspect.AreYou(list[0]))
            {
                Console.WriteLine(inspect.Execute(player, list.ToArray()));
            }
            else if (action.AreYou(list[0]))
            {
                Console.WriteLine(action.Execute(player, list.ToArray()));
            }
